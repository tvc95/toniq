...
export default function HistoryPage() {
  ...
  const avgHitRate = sessions.length
    ? Math.round(
        sessions.reduce((acc, s) => acc + (s.score / (s.total * 10)) * 100, 0) / sessions.length
      )
    : 0
      ...
      <section className="space-y-3">
        ...
        {!loading && sessions.length === 0 && (
          <div className="card text-center py-12 space-y-2">
            ...
            <p style={{ color: 'var(--color-text-secondary)' }}>Nenhuma sessão ainda</p>
          ...
          {sessions.map((s, i) => (
            <div key={i} className="card flex items-center justify-between p-4">
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{
                    ...
                    color: MODE_COLOR[s.mode as keyof typeof MODE_COLOR] ?? 'var(--color-primary)',
                  }}
                >
                  {MODE_LABEL[s.mode]}
                </span>
              ...
              <p
                ...
                style={{
                  color:
                    s.score >= s.total * 7 ? 'var(--color-secondary)' : 'var(--color-text-muted)',
                }}
              >
